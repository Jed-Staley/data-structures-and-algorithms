...
const notInFirstArray = (forbiddenValues, arr) => arr.filter((element) => forbiddenValues.includes(element) === false);
...
const getBaseStatGreaterThan = (arr, minBaseStat) => arr.filter((object) => object.baseStat > minBaseStat);
...
const getStatName = (arr, minBaseStat) => {
  let newArr = arr.filter((object) => object.baseStat > minBaseStat);
  return newArr.map((object) => object.stat.name);
};
...
const getCharactersWithoutChildren = arr => arr.filter((object) => object.children === undefined);
