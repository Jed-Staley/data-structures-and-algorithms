...
const transformToLis = obj => Object.entries(obj).map((subArr) => `<li>${subArr[0]}: ${subArr[1]}</li>`);
...
const count = (target, input) => input.reduce((occurrences, subArr) => subArr.reduce((occurrencesInSubArr, num) => (num === target) ? (occurrencesInSubArr + 1) : occurrencesInSubArr, 0) + occurrences, 0);
...
const totalSum = arr => arr.reduce((totalSum, subArr) => totalSum + subArr.reduce((subArrSum, num) => subArrSum + num, 0), 0);
...
const divisibleByFiveTwoToThePower = arr => arr.map((subArr) => subArr.filter((num) => ((num % 5 === 0) && (typeof num === 'number'))).map((num) => Math.pow(2, num)));
...
let findMaleAndFemale = data => data.reduce((finalStr, obj) => (/male/.test(obj.gender)) ? ((finalStr === '') ? obj.name : `${finalStr} and ${obj.name}`) : finalStr, '');
...
let findShortest = data => data.sort((obj1, obj2) => obj1.height - obj2.height)[0].name;
