...
const sortStarWarsCharacters = (starWarsArr) => {
  let workingArr = [...starWarsArr];
  let changesMade;
  do {
    changesMade = false;
    for (let i = 0; i < workingArr.length - 1; i++) {
      if (Number(workingArr[i].height) < Number(workingArr[i + 1].height)) {
        let entry1 = workingArr[i];
        workingArr[i] = workingArr[i + 1];
        workingArr[i + 1] = entry1;
        changesMade = true;
      }
    }
  } while (changesMade);
...
const joinArray = arr => arr.reduce((cumulativeStr, element) => cumulativeStr + ' ' + element);
